...
class Action {
	...
	function login2() {
		...
		if ($qry->num_rows > 0) {
			...
			if ($is_verified) {
				...
				// ✅ Merge guest cart into logged-in user's account
	...
	public function add_to_cart() {
		...
		if ($check->num_rows > 0) {
			return 2; // Already in cart
		}

		$save = $this->db->query("INSERT INTO cart (product_id, qty, user_id, client_ip) VALUES ('$product_id', '$qty', '$user_id', '$ip')");
		if ($save) return 1;
